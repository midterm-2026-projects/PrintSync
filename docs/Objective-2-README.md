...
| *Week 3* | *Day 2* | 	
Sales Transaction Logic Unit Testing
• Create unit tests for order creation logic.<br>• Test order item data processing and price calculation functions.<br>• Validate transaction data before saving.<br>
• Test helper functions for generating Order IDs and calculating totals.	• Vitest test files for sales logic.<br>• Unit test cases for order creation and order item handling.
• It should correctly generate an Order ID using the order creation function.<br>• It should validate and process order data correctly.<br>• It should correctly calculate item subtotals and total transaction amount.<br>• It should preserve the unit price value when creating order item records.|
