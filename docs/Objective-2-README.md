...
| *Week 3* | *Day 2* | Sales Database & API | • Create the non-functional Sales Model by defining and exporting query structures for orders and items.<br>• Develop the Sales Service layer to handle POS business logic, calculate totals, and generate unique Order IDs.<br>• Write isolated unit tests using vi.mock to verify service logic and validation rules. | • Sales Model (Exported, non-functional database queries).<br>• Sales Service (Core transaction processing logic).<br>• Unit test files for sales transaction logic. | • It should export defined data structures for handling orders and items from the model layer without executing live queries.<br>• It should correctly process calculations, generate unique IDs, and structure transaction data in the service layer.<br>• It should pass all unit tests by using vi.mock to intercept model dependencies and simulate database responses in isolation. |
