...
| **Week 4** | **Day 1** | Data Aggregation API | • Develop backend logic to aggregate sales data by specific dates.<br>• Provide a unit-testable AI-ready formatting layer (model + service) that prepares flat JSON payloads for the AI service. | • Sales aggregation service (core aggregation logic).<br>• AI-ready sales data formatter (model + service) that outputs flat Gemini-compatible JSON. | • It should return a single total sales figure for a specific date by summing all related rows in the orders table (unit-level aggregation logic).<br>• It should ensure the formatted AI payload is a flat JSON object compatible with Gemini prompt requirements (unit-level formatting logic).<br>• Formatter validation must throw `TypeError` for missing/invalid inputs.<br>• The formatting layer must have a model counterpart and unit tests must mock the model using `vi.mock` to verify service wiring. |
