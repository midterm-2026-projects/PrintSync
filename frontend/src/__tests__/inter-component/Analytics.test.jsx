...
describe('Analytics Page - AIInsightArea interactions', () => {
    ...
    it('clicking "Simulate AI Failure" displays red error message', async () => {
        vi.useFakeTimers();
        render(<Analytics />);
    ...
    it('error text replaces loading state and "Generating insights..." disappears', async () => {
        vi.useFakeTimers();
        render(<Analytics />);
        await waitFor(() => {
            expect(screen.getByTestId('revenue-total')).toBeInTheDocument();
    ...
    it('analyze button becomes re-enabled after simulated failure completes', async () => {
        vi.useFakeTimers();
        render(<Analytics />);
        await waitFor(() => {
            expect(screen.getByTestId('revenue-total')).toBeInTheDocument();
    ...
    it('insight box has overflow-y: auto and word-wrap: break-word styles for long text', async () => {
        render(<Analytics />);
        await waitFor(() => {
            expect(screen.getByTestId('revenue-total')).toBeInTheDocument();
        }, { timeout: 5000 });
